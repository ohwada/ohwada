...
public class MultiToSingle {
	...
	private final static String TAG = "MultiToSingle";
	 ...
	 */    
	public void convert( List<ParseList> list ) {
	...
	private void procTrack( int num ) {
		...
		while( true ) {
			...
			if ( mes == null ) break;				
			if ( mes.status == ParseMessage.STATUS_TEMPO ) {
				// テンポのときは、テンポを変更する
	...
	private class TrackProcedure {
		 ...
		 */			
		public TrackProcedure( ParseList list ) {
			mList = list;
			mPointer = 0;
			mPlayTime = 0 ; 
		}
		 ...
		 * @return ParseMessage	 
		 */
		public ParseMessage get() {
			ParseMessage r = mList.get( mPointer ); 
			mPointer ++;
			if ( r.status == ParseMessage.STATUS_DELTA_TIME ) {

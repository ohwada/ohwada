...
public class MidiMultiToSingle {
	...
	// debug
	private final static String TAG_SUB = "MidiMultiToSingle";

	// シングルトラックのMIDIメッセージのリスト
	private ArrayList<MidiMessage> mSingleList = null;
	...
	/*
	 * 変換する
	 * @oaram List<MidiParseList> list
	 */    
	public void convert( List<MidiParseList> list ) {
	...
	private void procTrack( int num ) {
		...
		while( true ) {
			...
			if ( mes == null ) break;				
			if ( mes.status == MidiParseMessage.ST_PARSE_TEMPO ) {
				// テンポのときは、テンポを変更する
	...
	private class TrackProcedure {
		...
		// MIDIメッセージのリスト
		private MidiParseList mList = null;
		...
		/*
		 * コンストラクタ
		 * @param MidiParseList list
		 */			
		public TrackProcedure( MidiParseList list ) {
			mList = list;
			mPointer = 0;
			mPlayTime = 0 ; 
		}
		...
		public MidiParseMessage get() {
			...
			mPointer ++;
			if ( r.status == MidiParseMessage.ST_PARSE_DELTA_TIME ) {

...
public class MidiParser {
	...
	// 解析されたMIDIのリスト
	private ParseList mParseList = null;
	
	// トラック毎のリスト
	private List<ParseList> mTrackList = null;
	...
	public int parse( byte[] bytes ) {
		...
		mReadPointer = 0;
		mReadEnd = 0;
		mTrackList = new ArrayList<ParseList>();		 				
	...
	/*
	 * 解析されたMIDIのリストを取得する
	 * @return List<ParseList>
	 */ 
	public List<ParseList> getList() {
		return mTrackList;
	}
	...
	private void procMsgDevice( int status_code, byte[] bytes ) {
		...
		mParseList.add( 
			new ParseMessage( status_code, bytes ) );
		String str = "";
	...
	private int procMsgTempo( byte status_byte, byte b1 ) {
		...
		mParseList.add( 
			new ParseMessage( ParseMessage.STATUS_TEMPO, bytes ) );	
	...
	private void procMsgOthersF0( byte status_byte, byte b1 ) {
       ...
       for ( int i=0; i<len; i++ ) {
        ...
        mParseList.add( 
        	new ParseMessage( ParseMessage.STATUS_OHERS, bytes ));
	...
	private void addDeltaTime() {
		...
		if ( time > 0 ) {
			mParseList.add( 
				new ParseMessage( ParseMessage.STATUS_DELTA_TIME, time ));
		}
	}
